...
buffalo_tracks <- as.data.frame(buffalo_utm) %>%
  dplyr::select(id = individual.local.identifier, x = coords.x1, y = coords.x2, ts = timestamp) %>%
...
table(table(ssf_cilla$step_id_))
...
#' Caveat: This habitat preference in general will not match the utilisation distribution of the animal (i.e. how much time it spends where).
#' # For the UD, a generic approach is to do simulations from the fitted model (see below).
#' See for more information Signer et al. (submitted; should be on arXiv soon) Simulating animal space use from fitted integrated Step-Selection Functions (iSSF)
...
terra::plot(habitat_map)
lines(ssf_cilla[, c("x1_", "y1_")])

#' Now zoom in
terra::plot(crop(habitat_map, ext(as_sf(cilla)) + 5000))
lines(ssf_cilla[, c("x1_", "y1_")])

#' The model is strongly driven by elevation
terra::plot(crop(buffalo_env[["elev"]], ext(as_sf(cilla)) + 5000))
lines(ssf_cilla[, c("x1_", "y1_")])
...
k2 <- amt::redistribution_kernel(m_hr, map = buffalo_env, start = amt:::make_start.steps_xyt(ssf_cilla[1, ]),
                            stochastic = TRUE, tolerance.outside = 0.01, as.rast = FALSE,
                            n.control = 1e3)
set.seed(2)
s2 <- amt::simulate_path(k2, n.steps = 1000)

terra::plot(crop(buffalo_env[["elev"]], extent_tracks(s2, cilla) + 2000))
lines(cilla)
...
terra::plot(crop(buffalo_env[["elev"]], extent_tracks(s3, cilla) + 2000))
lines(cilla)
