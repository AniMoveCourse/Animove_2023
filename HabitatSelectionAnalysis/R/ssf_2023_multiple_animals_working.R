...
buffalos <- buffalos |> mutate(steps = map(track, function(x) {
...
set.seed(23)
ssf_buffalos <- buffalos |> mutate(steps = map(steps, random_steps, n_control = 100,
                       sl_distr = combined_sl_distr,
                       ta_distr = combined_ta_distr)) |> unnest(cols = steps)
...
ssf_buffalos <- ssf_buffalos |> mutate(burst_ = paste(id, burst_),
...
#' Remove steps where there is no observed step (i.e. no case_ == TRUE) # I do not know why this does not work on ssf_buffalos...
ssf_df <- ssf_df |> dplyr::group_by(step_id_) |> dplyr::filter(any(case_))

#' ## Scaling predictors
#' We scale the predictors (mean of zero and standard deviation of one) because this helps with model convergence.
#' The values of the parameter estimates depend on the scaling of the predictors. So when we want to make predictions with the model,
#' we have to transform the maps of the predictors in the same way. The parameters for this transformation are stored in
#' the output of the scale function.
#'
scaled_env <- scale(ssf_df[,c("elev", "slope", "water_dist", "mean_NDVI", "var_NDVI")])
...
m_4 <- fit_clogit(ssf_df, case_ ~ (cos(ta_) + sl_ + log(sl_) +
                                     elev_scaled + mean_NDVI_scaled):id + strata(step_id_))
summary(m_4)

#' Alternatively, we can fit a model for each animal and then e.g. extract the parameter estimates
buffalo_models <- ssf_df |> nest(data = -"id") |> mutate(model = map(data, fit_clogit,
                                                               case_ ~ cos(ta_) + sl_ + log(sl_) +
                                                                 elev_scaled + mean_NDVI_scaled + strata(step_id_)))
buffalo_coef <- buffalo_models |> mutate(coef = map(model, function(x) as.data.frame(t(coef(x))))) |>
  dplyr::select(id, coef) |> unnest(col = coef)
...
#'
#' I generally had better convergence when using the optim optimizer and method BFGS
